-- ============================================
-- MERCADITO ONLINE PY - place_bid() VERSIÓN FINAL
-- Actualiza place_bid() con la versión final mejorada
-- que incluye configuración centralizada y retorno de bonus_applied
-- ============================================
-- Esta migración actualiza place_bid() con la versión final que:
-- - Usa get_bonus_time_config() para configuración centralizada
-- - Retorna información completa de bonus time (bonus_applied, bonus_new_end_time, etc.)
-- - Mantiene toda la lógica de validación y límites existente
-- ============================================
...
BEGIN
  ...
  -- Si el producto tiene auto_extend_seconds configurado, usarlo en lugar de la configuración global
...
DO $$
DECLARE
  v_uses_config BOOLEAN;
  v_returns_bonus BOOLEAN;
BEGIN
  -- Verificar que usa configuración centralizada
  SELECT prosrc LIKE '%get_bonus_time_config%' INTO v_uses_config
  FROM pg_proc
  WHERE proname = 'place_bid';
  
  -- Verificar que retorna información de bonus
  SELECT prosrc LIKE '%bonus_applied%' INTO v_returns_bonus
  FROM pg_proc
  WHERE proname = 'place_bid';
  
  IF NOT v_uses_config THEN
    RAISE WARNING 'place_bid() NO usa get_bonus_time_config()';
  END IF;
  
  IF NOT v_returns_bonus THEN
    RAISE WARNING 'place_bid() NO retorna información de bonus';
  END IF;
  
  IF v_uses_config AND v_returns_bonus THEN
    RAISE NOTICE '✅ place_bid() actualizada correctamente:';
    RAISE NOTICE '   - Usa configuración centralizada (get_bonus_time_config)';
    RAISE NOTICE '   - Retorna información de bonus (bonus_applied, bonus_new_end_time)';
  END IF;
END $$;
